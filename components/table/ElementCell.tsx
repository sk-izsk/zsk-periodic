...
export default function ElementCell({ element, dimmed, highlighted, onClick }: Props) {
  ...
  return (
    <div
      ...
      title={element.name}
    >
      <span style={{ fontSize: 10, color: 'rgba(255,255,255,.8)', alignSelf: 'flex-start', paddingLeft: 6, lineHeight: 1 }}>
        {element.n}
      </span>
      <span style={{ fontSize: 36, fontWeight: 500, color: '#fff', lineHeight: 1.05 }}>
        {element.sym}
      </span>
      <span style={{ fontSize: 9, color: 'rgba(255,255,255,.85)', lineHeight: 1, overflow: 'hidden', maxWidth: 64, textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: 2 }}>
        {element.name}
      </span>
    </div>
  );
