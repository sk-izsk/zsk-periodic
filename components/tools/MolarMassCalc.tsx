  ...
  return (
    <div className="rounded-xl p-5" style={{ background: 'var(--color-bg2)', border: '0.5px solid var(--color-border)' }}>
      ...
      <div className="flex gap-2 mb-4">
        ...
        />
        <button onClick={run} className="px-4 py-2 rounded-lg text-sm bg-blue-600 text-white hover:bg-blue-700">
          Calculate
        </button>
      </div>
      ...
      {result && !result.error && (
        <div>
          <div className="text-2xl font-medium mb-3">{result.total} <span style={{ fontSize: 14, color: 'var(--color-muted)' }}>g/mol</span></div>
          <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '0.5px solid var(--color-border)', color: 'var(--color-muted)' }}>
            ...
            <tbody>
              {result.breakdown.map(row => (
                <tr key={row.element} style={{ borderBottom: '0.5px solid var(--color-border)' }}>
                  ...
                  <td style={{ textAlign: 'right', padding: '4px 0', color: 'var(--color-muted)' }}>{row.mass}</td>
                  <td style={{ textAlign: 'right', padding: '4px 0' }}>{row.contribution.toFixed(4)}</td>
                </tr>
              ))}
            </tbody>
          </table>
