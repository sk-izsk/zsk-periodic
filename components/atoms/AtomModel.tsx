...
const SHELL_COLORS = ['#ff6b6b', '#ff9f43', '#ffe66d', '#7bed9f', '#70a1ff', '#a29bfe', '#f78fb3']
  ...
  if (total === 1) {
    ...
    if (total > 20) {
      ...
      for (const p of particles) {
        maxDist = Math.max(maxDist, p.pos.length())
      }
      const cutoff = maxDist - pRadius * 3
      if (cutoff > 0) {
        particles = particles.filter((p) => p.pos.length() >= cutoff)
      }
    }
  }
  return particles
...
function Shell({
  ...
  const angles = useRef<number[]>(
    Array.from({ length: electrons }, (_, e) => (e / electrons) * Math.PI * 2),
  )
  ...
  const trailGeos = useMemo(
    () =>
      Array.from(
        { length: TRAIL_COUNT },
        (_, t) => new THREE.SphereGeometry(Math.max(0.05, 0.18 - t * 0.012), 6, 6),
      ),
    [],
  )
          ...
          <group key={e}>
            ...
            {trailGeos.map((tGeo, t) => (
              <mesh
                key={t}
                ref={(m) => {
                  if (!trailRefs.current[e]) {
                    trailRefs.current[e] = []
                  }
                  trailRefs.current[e][t] = m
                }}
                geometry={tGeo}
...
function configToShells(config: string): number[] {
  ...
  for (const [sym, shells] of Object.entries(NG_SHELLS)) {
    if (config.startsWith(sym)) {
      base = [...shells]
      rest = config.slice(sym.length).trim()
      break
    }
  }
  const counts = [...base]
